...
occupied slots (from PostgreSQL) from the daily time grid.

The time grid is fetched dynamically from the database so it
automatically adapts when a new timetable PDF is imported.
"""
...
# ─── DAILY TIME GRID ─────────────────────────────────────────────────────────
# Fallback grid used only when the DB has no data (e.g. between imports).
# The primary source is _get_all_slots() which reads from the DB.

_FALLBACK_SLOTS: List[str] = [
    "08:00-08:50",
    "08:55-09:45",
    "09:50-10:40",
    "10:45-11:35",
...
# Cache for dynamic slots (refreshes every 5 minutes)
_cached_all_slots: Optional[List[str]] = None
_cached_all_slots_ts: float = 0
...
def _normalize_slot(slot: str) -> str:
    cleaned = slot.replace("–", "-").replace("—", "-").replace(":-", "-")
...
def _parse_time_range(slot: str) -> Optional[tuple]:
    ...
    # Convert PM hours (1:00 PM to 7:00 PM) to 24-hour time
    if start_h < 7:
        start_h += 12
    if end_h < 7:
        end_h += 12
...
def _get_all_slots() -> List[str]:
    """Return the daily time-slot grid.

    Primary:  unique time values from university_timetable (cached 5 min).
    Fallback: hardcoded _FALLBACK_SLOTS when the DB is empty or unreachable.
    """
    global _cached_all_slots, _cached_all_slots_ts

    now = _time_module.time()
    if _cached_all_slots and (now - _cached_all_slots_ts) < 300:
        return _cached_all_slots
    ...
    try:
        conn = _get_connection()
        cur  = conn.cursor()
        cur.execute(f"SELECT DISTINCT {TIME_COL} FROM {TABLE}")
        rows = cur.fetchall()
        cur.close()
        conn.close()

        slots = list(set(
            _normalize_slot(row[0]) for row in rows if row and row[0]
        ))
        slots = sorted(slots, key=_slot_sort_key)

        if slots:
            _cached_all_slots    = slots
            _cached_all_slots_ts = now
            return slots
    except Exception as e:
        print(f"  [FreeSlot] Error fetching slots from DB: {e}")
...
def get_teacher_free_slots(teacher_name: str, day: str, language_hint: str = "english") -> str:
    ...
    free = _compute_free(_get_all_slots(), occupied or [])
