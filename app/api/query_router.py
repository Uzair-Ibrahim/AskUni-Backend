...
_SECTION_PATTERN = re.compile(
    r"\b(B?S?CS|BSCS|BCS|BSSE|BSE|BSAI|BSDS|BSCY|BCY|BAI|BDS|BBA|MBA|MS[A-Z]{0,3})"
    r"\s*[-–]?\s*0?(\d[A-Z]?)\b",
    re.IGNORECASE,
)
...
def extract_section(query: str) -> str:
    """
    Extract a section identifier like 'BSCS-5A' from a query.
    Handles informal formats: bcs4b, bscs04b, BCS-4E, bscs 4b
    Normalizes to a format suitable for ILIKE DB search.
    """
    ...
    # Normalize: PROGRAM-NUMBER (e.g., BCS-4B)
