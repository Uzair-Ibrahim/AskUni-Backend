...
The KNOWLEDGE category is further sub-classified within rag_pipeline.py
into FACULTY, ADMISSION, PROGRAM, FEE, or GENERAL.
"""
...
_DAY_PATTERN = re.compile(
    ...
    r"monady|moday|mondy|tuesay|tusday|wednsday|thurday|frday|satday|"
    r"kal|aaj|tomorrow|today)\b",
    re.IGNORECASE,
)

_SECTION_PATTERN = re.compile(
    r"\b(B?S?CS|BSCS|BCS|BSSE|BSE|BSAI|BAI|BSDS|BDS|BSCY|BCY|BCE|BEE|BSEE|BFT|BSBA|BBA|MBA|MS[A-Z]{0,3})"
    r"\s*[-–]?\s*0?(\d[A-Z0-9]*)\b",
    re.IGNORECASE,
)
...
def extract_section(query: str) -> str:
    """
    Extract a section identifier like 'BSCS-5A', 'BAI-1A', 'BCS-1N1', 'BFT-1H' from a query.
    Handles informal formats: bcs4b, bscs04b, BCS-4E, bscs 4b, bft1a, bai-1a, bcs1n1
    Normalizes to a format suitable for ILIKE DB search (e.g. BCS-1N1).
    """
    match = _SECTION_PATTERN.search(query)
    if not match:
        return ""
    program = match.group(1).upper()
    number_part = match.group(2).upper()
    # Normalize: PROGRAM-NUMBER (e.g., BCS-4B, BAI-1A, BCS-1N1)
    return f"{program}-{number_part}".strip()
        ...
        if qt == QueryType.KNOWLEDGE:
            try:
                ...
                if q_info.get('domain_filter'):
                    desc += f" (filter: {q_info['domain_filter']})"
            except Exception:
                pass
